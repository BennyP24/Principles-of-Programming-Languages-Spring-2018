...
import { isArray, isBoolean, isEmpty, isNumber, isString, isLazyVarDecl, LazyVarDecl, makeLazyVarDecl, isVarDecl } from "./L3-ast";
...
import { makeAppExp, makeBoolExp, makeIfExp, makeLitExp, makeNumExp, makeProcExp, makeStrExp,
         makeVarDecl, makeVar, makeVarRef } from "./L3-ast";
...
const evalIfNotLazy = (arg: CExp, env: Env): Value | CExp | Error =>{
    console.log(arg);
    return !isLazyVarDecl(arg) ? L3applicativeEval(arg, env) :
    arg;
}

const evalIfLazy = (arg: CExp | Value, env: Env): Value | Error =>
    isCExp(arg) ? L3applicativeEval(arg, env):
    arg;

const L3applyProcedure = (proc: Value | Error, args: (CExp | Value)[], env: Env): Value | Error =>{
    if (isError(proc)) {
        return proc;
    }else if(isPrimOp(proc)){
        const argVals: Array <Value | Error> = map(L3applicativeEval, args);
...
const valueToLitIfNotLazyExp = (v: Value | CExp): NumExp | BoolExp | StrExp | LitExp | PrimOp | ProcExp | CExp =>
    isCExp(v)? v :
    valueToLitExp(v);

// @Pre: none of the args is an Error (checked in applyProcedure)
const applyClosure = (proc: Closure, args: (CExp | Value)[], env: Env): Value | Error => {
    let vars = map((v: VarDecl) => v.var, proc.params);
    let body = renameExps(proc.body);
    let litArgs = map(valueToLitIfNotLazyExp, args);
...
export const renameExps = (exps: CExp[]): CExp[] => {
    ...
    const replaceProc = (e: ProcExp): ProcExp => {
        const oldArgs = map((arg: LazyVarDecl | VarDecl): string => arg.var, e.args);
        const newArgs = map(varGen, oldArgs);
        const newVarDecls = map((x: [string, LazyVarDecl | VarDecl]) => isVarDecl(x[1]) ? makeVarDecl(x[0]) : makeLazyVarDecl(x[0]), zip(newArgs, e.args));
        const newBody = map(replace, e.body);
        return makeProcExp(newVarDecls,
