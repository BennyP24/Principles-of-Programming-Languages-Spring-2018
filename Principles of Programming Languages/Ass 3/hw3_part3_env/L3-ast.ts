...
export interface ProcExp {tag: "ProcExp"; args: (LazyVarDecl| VarDecl)[], body: CExp[]; };
...
export const parseL3Sexp = (sexp: any): Parsed | Error =>{
    return isEmpty(sexp) ? Error("Parse: Unexpected empty") :
...
const parseL3Compound = (sexps: any[]): Parsed | Error =>{
    return isEmpty(sexps) ? Error("Unexpected empty sexp") :
...
export const makeVar = (v: string | string[]): VarDecl | LazyVarDecl => 
    isString(v) ? makeVarDecl(v) :
    makeLazyVarDecl(v[0]);

const parseProcExp = (sexps: any[]): ProcExp | Error =>
    safeFL((body: CExp[]) => makeProcExp( map(makeVar, sexps[1]), body))
        (map(parseL3CExp, rest(rest(sexps))));

export const parseDecls = (sexps: any[]): (LazyVarDecl | VarDecl)[] | Error =>
    allT(((x) : x is string | Array<string> => isString(x) || isArray(x)), sexps) ? map(makeVar, sexps) :
...
const safeMakeBindings = (decls: (LazyVarDecl | VarDecl)[] | Error, vals: Array<CExp | Error>): Binding[] | Error =>
    isError(decls) ? decls :
