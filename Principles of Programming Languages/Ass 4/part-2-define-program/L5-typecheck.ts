...
export const typeofExp = (exp: Parsed | Error, tenv: TEnv): TExp | Error =>{
    return isNumExp(exp) ? typeofNum(exp) :
...
export const typeofDefine = (exp: DefineExp, tenv: TEnv): TExp | Error => {
    let Tval = typeofExp(exp.val, tenv);
    if(isError(Tval)){
        return Tval;
    }else{
        let constraint = checkEqualType(exp.var.texp, Tval, exp);
        return isError(constraint) ? constraint : makeVoidTExp();
    }
};
...
export const typeofProgram = (exp: Program, tenv: TEnv): TExp | Error => {
    return typeOfProgramEzer(exp.exps, tenv);
};

export const typeOfProgramEzer = (exps: Exp[], tenv: TEnv): TExp | Error =>{
    if (isError(typeofExp(first(exps), tenv))){
        return typeofExp(first(exps), tenv);
    }else if(isEmpty(rest(exps))){
        return typeofExp(first(exps), tenv);
    }else if(isDefineExp(first(exps))){
        let defineExpVar = first(exps).var.var;
        let defineExpTval = typeofExp(first(exps).val, tenv);
        if(isError(defineExpTval)){
            return defineExpTval;
        }else{
        let newTenv = makeExtendTEnv([defineExpVar], [defineExpTval], tenv);
        return isError(typeofExp(first(exps), tenv)) ? typeofExp(first(exps), tenv) :
        typeOfProgramEzer(rest(exps), newTenv);
        }
    }else
    return typeOfProgramEzer(rest(exps), tenv);
};