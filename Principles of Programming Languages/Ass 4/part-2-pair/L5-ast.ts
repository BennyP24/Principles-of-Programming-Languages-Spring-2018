...
export interface PairExp {tag: "PairExp"; type1: CExp; type2: CExp; };
export const makePairExp = (type1: CExp, type2: CExp): PairExp => ({tag: "PairExp", type1: type1, type2: type2});
...
export const parseLitExp = (sexps: any[]): PairExp | Error =>{
    let arr = second(sexps);
    if(arr.length === 3 && arr[1] === '.'){
        return isError(parseCExp(arr[2])) ? parseCExp(arr[2]) :
        isError(parseCExp(arr[0])) ? parseCExp(arr[0]) :
        makePairExp(parseCExp(arr[0]),parseCExp(arr[2]));
    }else{
        return Error();
    }
}
...
export const unparse = (e: Parsed | Error): string | Error =>
    ...
    isPairExp(e) ? `(Pair ${unparse(e.type1)} ${unparse(e.type2)})` :
