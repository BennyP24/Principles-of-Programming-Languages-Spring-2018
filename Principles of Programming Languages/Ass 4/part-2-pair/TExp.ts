...
export type CompoundTExp = PairTExp |ProcTExp | TupleTExp;
export const isCompoundTExp = (x: any): x is CompoundTExp => isPairTExp(x) || isProcTExp(x) || isTupleTExp(x);
...
// PairTExp
export type PairTExp = { tag: "PairTExp"; type1: TExp, type2: TExp};
export const makePairTExp = (type1: TExp, type2: TExp): PairTExp =>
    ({tag: "PairTExp", type1: type1, type2: type2});
...
*/
const parseCompoundTExp = (texps: any[]): PairTExp | ProcTExp | Error => {
    const isPair = texps.indexOf('Pair')
    if(isPair === -1){
    ...
    safeMakeProcTExp(parseTupleTExp(texps.slice(0, pos)),
        ...
        if(texps.length !== 3){
            return Error(`Expected 2 params in pair type expression`); 
        }else{
            let type1 = parseTExp(texps[1]);
            let type2 = parseTExp(texps[2]);
            return isError(type1) ? type1 :
            isError(type2) ? type2 : makePairTExp(type1, type2);       
        }
    }
};
...
export const unparseTExp = (te: TExp | Error): string | Error => {
    ...
    const up = (x: TExp | Error): string | string[] | Error =>
        ...
        isPairTExp(x) ? ['Pair', unparseTExp(x.type1), unparseTExp(x.type2)] :
        ["never"];
...
export const matchTVarsInTE = <T1, T2>(te1: TExp, te2: TExp,
    ...
    (isPairTExp(te1) || isPairTExp(te2)) ? makeTvarsinTPairs(te1, te2, succ, fail) :
...
const makeTvarsinTPairs = <T1, T2>(te1: TExp, te2: TExp,
                                  succ: (mapping: Array<Pair<TVar, TVar>>) => T1,
                                  fail: () => T2): T1 | T2 =>
    (isPairTExp(te1) && isPairTExp(te2)) ? matchTVarsInTEs([te1.type1, te1.type2], [te2.type1, te2.type2], succ, fail) : fail();
