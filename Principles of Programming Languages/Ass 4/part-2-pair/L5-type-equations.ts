...
export const makeEquationFromExp = (exp: A.Exp, pool: Pool): Equation[] =>
    ...
    A.isAppExp(exp) ? [makeEquation(inPool(pool, exp.rator),
                                   T.makeProcTExp(R.map((e) => inPool(pool, e), exp.rands),
                                                  inPool(pool, exp)))] :
    A.isPairExp(exp) ? [makeEquation(inPool(pool, exp), T.makePairTExp(inferType(exp.type1), inferType(exp.type2)))] :
...
export const splitEquation = (eq: Equation): Equation[] =>
    ...
    (T.isPairTExp(eq.left) && T.isPairTExp(eq.right)) ?
        R.zipWith(makeEquation,
                  [eq.left.type1, eq.left.type2],
                  [eq.right.type1, eq.right.type2]) :
    [];
