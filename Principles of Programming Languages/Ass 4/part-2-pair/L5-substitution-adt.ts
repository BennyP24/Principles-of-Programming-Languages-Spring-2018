...
export const checkNoOccurrence = (tvar: TVar, te: TExp): true | Error => {
    const check = (e: TExp): true | Error =>
        ...
        isPairTExp(e) ? isError(check(e.type1)) ? Error(getErrorMessages([e.type1])) : 
                        isError(check(e.type2)) ? Error(getErrorMessages([e.type2])) : 
                        true :
...
export const applySub = (sub: Sub, te: TExp): TExp =>
    ...
    isPairTExp(te) ? makePairTExp(applySub(sub, te.type1),applySub(sub, te.type2)) :
    te;
